...
\begin{equation}
    ...
    \label{eq:zscore}
\end{equation}
with $\varepsilon = 10^{-8}$ to avoid division by zero. Any frame whose $z_t$ exceeds a threshold $\tau$ is treated as an anomaly. The predicted accident frame is:
\begin{equation}
    t^{*}_{\text{frame}} =
    \begin{cases}
        \min\{t : z_t > \tau\} & \exists\, t \text{ s.t. } z_t > \tau \\
        \arg\max_t\, z_t & \text{otherwise}
    \end{cases}
    \label{eq:peak}
\end{equation}
with $\tau = 1.5$. Taking the first threshold-crossing frame favors early detection. When no frame crosses the threshold, we take the frame with the highest $z_t$ as a fallback. Time in seconds is $t^{*} = t^{*}_{\text{frame}} / f$.
...
Processing begins at frame $\lfloor N/3 \rfloor$ because collisions generally happen after a period of normal driving. For each pair of consecutive frames within a 30-frame window, we run the Farneback dense optical flow algorithm~\cite{farneback2003two} at $320 \times 180$ resolution. Farneback estimates per-pixel displacement using quadratic polynomial expansions and a multi-scale Gaussian pyramid. We set pyramid scale to $0.5$, three levels, window size $15$, three iterations, and polynomial parameters $n = 5$, $\sigma_p = 1.2$.
...
\begin{equation}
    ...
    \label{eq:magmap}
\end{equation}
The impact location is the weighted centroid of $M$, normalized to unit coordinates:
\begin{equation}
...
For each type $k \in \mathcal{K}$, we write three natural language descriptions $\{p_k^1, p_k^2, p_k^3\}$ that characterize the collision from a bystander perspective (Table~\ref{tab:prompts}). Using several prompts per class reduces the influence of any one particular wording, consistent with the prompt ensembling approach studied by Radford et al.~\cite{radford2021learning} and Zhou et al.~\cite{zhou2022learning}. Each prompt is encoded by the CLIP text encoder $\phi_{\text{text}}$, L2-normalized, and then averaged:
\begin{equation}
    \mathbf{t}_k = \frac{1}{3} \sum_{j=1}^{3} \frac{\phi_{\text{text}}(p_k^j)}{\|\phi_{\text{text}}(p_k^j)\|}
    \label{eq:textfeat}
\end{equation}
These vectors are computed once and cached before processing any video.

At inference time, we extract four video frames centered on the predicted accident time $t^{*}$ and feed them into the CLIP visual encoder (ViT-B/32). Each embedding is L2-normalized, and we average the four into a single representation:
\begin{equation}
    \mathbf{v} = \frac{1}{4} \sum_{i=1}^{4} \frac{\phi_{\text{img}}(I_{t_i})}{\|\phi_{\text{img}}(I_{t_i})\|}
    \label{eq:imgfeat}
\end{equation}
...
\centering
\small
\caption{Prompt templates for collision type classification. Three prompts per class are averaged into one text embedding.}
\label{tab:prompts}
\begin{tabular}{@{}ll@{}}
\toprule
