...
On a 10-video calibration subset drawn from the synthetic split, we measure per-component mean scores of $\overline{\mathcal{T}} = 0.438$ for temporal localization, $\overline{\mathcal{S}} = 0.168$ for spatial localization, and $\overline{\mathcal{C}} = 0.0$ for classification. The zero classification score is expected: CLIP consistently predicts t-bone for this subset, while the ground truth labels are head-on. Because the harmonic mean equals zero whenever any component is zero, the composite score $\overline{\mathcal{H}} = 0.0$ on calibration. This confirms that classification accuracy is the dominant bottleneck in the current pipeline.
